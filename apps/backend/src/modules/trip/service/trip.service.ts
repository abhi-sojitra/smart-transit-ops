...
export class TripService {
  ...
  async dispatchTrip(id: string, user?: JwtPayload) {
    ...
    await Promise.all([
      ...
      this.driverService.updateStatus(driverId, DriverStatus.ON_TRIP),
    ]);
  ...
  async completeTrip(id: string, dto: CompleteTripDto, user?: JwtPayload) {
    ...
    await Promise.all([
      ...
      this.driverService.updateStatus(driverId, DriverStatus.AVAILABLE),
    ]);
  ...
  private async assertCanReadTrip(trip: TripDocument, user?: JwtPayload) {
    ...
    if (user.roles.includes(RoleCode.OPERATOR)) {
      const driver = await this.driverService.findByUserId(user.sub);
      const tripDriverId = this.refId(trip.driverId);
      if (driver && String(driver._id) === tripDriverId) return;
